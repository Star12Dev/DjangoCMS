...
from form_addon.models import  PluginPosition as FormPluginPosition
from form_addon.models import InputPluginPosition, LabelPluginPosition, FormBtnPluginPosition

from blero_container.models import  BleroContainer
from blero_container.models import PluginPosition as BleroContainerPluginPosition

from log_terminal.models import  LogTerminal
from log_terminal.models import PluginPosition as LogTerminalPluginPosition

from blero_grid.models import BleroGrid
from blero_grid.models import PluginPosition as BleroGridPluginPosition

from task_tracker.models import TaskHolder
from task_tracker.models import PluginPosition as TaskHolderPluginPosition
...
from dash_docs_app.lib.DocsConstruction import BuildDoc
...
handler = logging.FileHandler('dashboards_app.log')
...
class DashboardDetail(AppConfigMixin, AppHookCheckMixin, PreviewModeMixin,
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            file_name='resources/user_logs/'+str(author).replace(" ", "")+active_dashboard.slug+str(active_dashboard.id)+".log"
        ...
        if os.path.isfile(file_name)==False:
            f=open(file_name,'w')
            f.write("#this file should hold the functions that can be selected in each dashboard, do not delete or rename this file")
            f.close()
...
def plugin_position(request, plugin_type):
    if request.method == 'POST':
        ...
        if plugin_type=="FormPlugin":
            obj, created = FormPluginPosition.objects.update_or_create(
                model=FormPlugin.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
                    'height': height,
        ...
        if plugin_type=="BleroContainer":

            obj, created = BleroContainerPluginPosition.objects.update_or_create(
                model=BleroContainer.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
                    'height': height,
        ...
        if plugin_type=="LogTerminal":

            obj, created = LogTerminalPluginPosition.objects.update_or_create(
                model=LogTerminal.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
                    'height': height,
        ...
        if plugin_type=='FormInput':
            obj, created = InputPluginPosition.objects.update_or_create(
                model=FormInputs.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
                    'height': height,
        ...
        if plugin_type=='FormLabel':
            obj, created = LabelPluginPosition.objects.update_or_create(
                model=FormInputs.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
        ...
        if plugin_type=='FormBtn':
            obj, created = FormBtnPluginPosition.objects.update_or_create(
                model=FormPlugin.objects.get(pk=pk),
                defaults={
                    'is_resized': True,
                    'width': width,
        ...
        try:
            if plugin_type == 'BleroGrid':
                obj, created = BleroGridPluginPosition.objects.update_or_create(
                    model=BleroGrid.objects.get(pk=pk),
                    defaults={
                        'is_resized': True,
                        'width': width,
                        'height': height,
            ...
            logger.exception("BleroGrid not resized")

        try:
            if plugin_type == 'TaskHolder':
                obj, created = TaskHolderPluginPosition.objects.update_or_create(
                    model=TaskHolder.objects.get(pk=pk),
                    defaults={
                        'is_resized': True,
                        'width': width,
                        'height': height,
            ...
            logger.exception("TaskHolder not resized")
        ...
        if plugin_type == "FormPlugin":
            position = FormPluginPosition.objects.filter(model=FormPlugin.objects.get(pk=pk))
            # FormPluginPosition.objects.filter(model=FormPlugin.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == "BleroContainer":
            position = BleroContainerPluginPosition.objects.filter(model=BleroContainer.objects.get(pk=pk))
            # BleroContainerPluginPosition.objects.filter(model=BleroContainer.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == "LogTerminal":
            position = LogTerminalPluginPosition.objects.filter(model=LogTerminal.objects.get(pk=pk))
            # LogTerminalPluginPosition.objects.filter(model=LogTerminal.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == 'FormInput':
            position = InputPluginPosition.objects.filter(model=FormInputs.objects.get(pk=pk))
            # InputPluginPosition.objects.filter(model=FormInputs.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == 'FormLabel':
            position = LabelPluginPosition.objects.filter(model=FormInputs.objects.get(pk=pk))
            # LabelPluginPosition.objects.filter(model=FormInputs.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == 'FormBtn':
            position = FormBtnPluginPosition.objects.filter(model=FormPlugin.objects.get(pk=pk))
            # FormBtnPluginPosition.objects.filter(model=FormPlugin.objects.get(pk=pk)).update(is_resized=False)

        if plugin_type == 'FormBtn':
            position = BleroGridPluginPosition.objects.filter(model=BleroGrid.objects.get(pk=pk))

        if plugin_type == 'TaskHolder':
            position = TaskHolderPluginPosition.objects.filter(model=TaskHolder.objects.get(pk=pk))
